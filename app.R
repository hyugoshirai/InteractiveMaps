...
server <- function(input, output, session) {
  ...
  df_bounds <- reactive({
    ...
    bounds <- input$map_bounds # get the boundaries of the actual map defined by the zoom level
    latRng <- range(bounds$north, bounds$south) # get the latitude values of north and south boundaries
    lngRng <- range(bounds$east, bounds$west) # get the longitude values of east and west boundaries

    subset(df_sites, # subset df acording to the coordinates of the boundary defined by zoom level
           decimalLatitude >= latRng[1] & decimalLatitude <= latRng[2] &
             decimalLongitude >= lngRng[1] & decimalLongitude <= lngRng[2])
  })

  output$species_in_area <- gt::render_gt({ # Creates a list to display the most observed species in area. Alternatively
    # we could use to display a graph or other information
    df_bounds() %>%
      select(species_list) %>%
  ...
  output$map <- renderLeaflet({
    leaflet(options = leafletOptions(preferCanvas = TRUE)) %>%
      ...
      fitBounds(lng1 = -75, lat1 = -15, lng2 = -45, lat2 = 5)  # Amazon region bounds
  })
  ...
  output$species_list_text <- renderUI({ 
    if(!is.null(input$map_marker_click)){
      ...
      # We will access the information of the markers that user clicks on map
      marker_month <- lubridate::month(as.Date(glue::glue("2021-{marker_data[4]}-01")),label=TRUE,abbr = FALSE)
    species_list <- df_sites %>%
      filter(site_id == input$map_marker_click$id) %>% # access the id of the input, of each marker
      select(species_list) %>%
      unlist() %>%
      str_trim() %>%
      str_split(",") %>%
  ...
  count_palet <- colorBin(palette = "Dark2",bins = 3, pretty=TRUE, # bins define how many equal classes the counting will be 
                          # break in the legend. We can define as quantiles or other manually break
                          domain = range(df_sites$species_count))

  # reactive map update
  observe({
    leafletProxy("map", data = df_react()) %>% #This prevents the entire map to be rendered every time we filter data
      clearMarkerClusters() %>%
      clearShapes() %>%
      clearMarkers() %>%
      clearControls() %>%
      ...
      # The clusterOptions is defining how the points will be clustered
      # layerId will define the id of each marker, it 
      # is a column on the data frame
